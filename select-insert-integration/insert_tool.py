...
def build_insert_query(table: str, data: Dict[str, Any], on_conflict: Optional[str], 
                      returning: Optional[List[str]]) -> Tuple[str, List[Any]]:
    ...
    if on_conflict == 'ignore':
        ...
        query += f" ON CONFLICT DO UPDATE SET {', '.join(update_cols)}"
...
def mcp_insert_tool(table: str, data: Dict[str, Any], on_conflict: Optional[str] = None, 
    ...
    try:
        ...
        with get_connection(db_config) as conn:
            ...
            query, values = build_insert_query(table, data, on_conflict, returning)
